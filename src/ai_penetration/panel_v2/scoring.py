...
def run(rel_dir: Path, bench: bool = False) -> None:
    ...
    if not np.array_equal(np.sort(firm_job_arr), jobs):
        raise RuntimeError(
            f"firm 表与岗位全集不一一对应: firm={len(firm_job_arr)} jobs={len(jobs)}"
        )
    ...
    if n_y > 16:
        raise RuntimeError("留一 triple 键打包假设 yidx<16（M1 防扩年后静默错配）")
            ...
            for st in ("raw", "smoothed"):
                ...
                pq.write_table(
                    pa.Table.from_pandas(pd.DataFrame({
                        "job_id": jobs, "year": job_year + ymin,

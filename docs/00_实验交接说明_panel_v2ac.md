...
|---|---|---|
| **>0.15（主指标：AI 岗位占比）** | **1.98%** | 带内金标准真值率 70-92%，与 Babina 美国峰值（2-4%）同数量级 |
| >0.10 | 3.92% | 中间档 |
| >0.05（暴露率，非岗位占比） | 10.81% | 0.05-0.15 增量带真值率仅 4%，仅作描述性 |
...
**主指标已切换为 >0.15（净 AI 岗占比 2024≈1.6-2.1%，Babina 同数量级）；
>0.05 更名"AI 技能暴露率"使用。决策已于 2026-09-09 落地（config/panel_v2.yaml §2.5）。**
...
annual 暴露指标（>0.05）判定翻转仅 **0.082%**（10.79% vs 10.81%，暴露度非企业模板自推）；
...
1. ~~主指标切换 >0.15 的发布重算~~ ✅ 已执行（2026-09-09：config 语义、scoring 日志、本文档 §2.5/§3 全部更新，分类表三列本就齐备零重跑）
